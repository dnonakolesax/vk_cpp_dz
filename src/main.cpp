...
std::map<std::string, std::string> processArgs(int argc, char* argv[]) {
    ...
    if (argc < 10) {  // имя программы + 4 ключа + 4 файла + имя режиссёра
        std::cerr << "Usage: ./dz1 <director name> "
                     "--<file name>-path <path to file>." << std::endl;
        std::cerr << "Files required: name.basics.tsv, title.akas.tsv, "
                     "title.basics.tsv, title.crew.tsv" << std::endl;
    ...
    for (size_t i = 2; i < argc; i+=2) {
        ...
        if (arg == "--name-basics-path")
            progArgs["namesPath"] = std::string(argv[i+1]);
        else
        if (arg == "--title-akas-path")
            progArgs["akasPath"] = std::string(argv[i+1]);
        else
        if (arg == "--title-basics-path")
            progArgs["basicsPath"] = std::string(argv[i+1]);
        else
...
bool checkFiles(std::map<std::string, std::string> progArgs) {
    ...
    if (header.size() != NBASICS_COLUMNS_AMNT) {
        throw std::invalid_argument("File name.basics.tsv is corrupted:"
                                    " not enough columns!");
    }
    if (header[NBASICS_NCONST_INDX] != NBASICS_NCONST_COLNAME ||
        header[NBASICS_NAME_INDX] != NBASICS_NAME_COLNAME) {
        throw std::invalid_argument("File name.basics.tsv is corrupted:"
                                     " wrong column contents.");
    }
    nameBasics.close();
    ...
    if (header.size() != TAKAS_COLUMNS_AMNT) {
        throw std::invalid_argument("File title.akas.tsv is corrupted:"
                                    " not enough columns");
    }
    if (header[TAKAS_TCONST_INDX] != TAKAS_TCONST_COLNAME ||
        header[TAKAS_LOCTITLE_INDX] != TAKAS_LOCTITLE_COLNAME ||
        header[TAKAS_LOCALE_INDX] != TAKAS_LOCALE_COLNAME) {
        throw std::invalid_argument("File title.akas.tsv is corrupted:"
                                    "wrong column contents");
    }
    titleAkas.close();
    ...
    if (header.size() != TBASICS_COLUMNS_AMNT) {
        throw std::invalid_argument("File title.basics.tsv is corrupted:"
                                    " not enough columns");
    }
    if (header[TBASICS_TCONST_INDX] != TBASICS_TCONST_COLNAME ||
        header[TBASICS_TITLETYPE_INDX] != TBASICS_TITLETYPE_COLNAME ||
        header[TBASICS_ISADULT_INDX] != TBASICS_ISADULT_COLNAME ||
        header[TBASICS_ORIGTITLE_INDX] != TBASICS_ORIGTITLE_COLNAME) {
        throw std::invalid_argument("File title.basics.tsv is corrupted:"
                                    " wrong column contents");
    }
    titleBasics.close();
    ...
    if (header.size() != TCREW_COLUMNS_AMNT) {
        throw std::invalid_argument("File title.crew.tsv is corrupted:"
                                    " not enough columns");
    }
    if (header[TCREW_TCONST_INDX] != TCREW_TCONST_COLNAME ||
        header[TCREW_DIRECTORS_INDX] != TCREW_DIRECTORS_COLNAME) {
        throw std::invalid_argument("File title.crew.tsv is corrupted:"
                                    " wrong column contents");
    }
    titleCrew.close();
...
std::vector<movie_t> getRussianLocalNames(std::map<std::string, std::string> progArgs) {
    ...
    auto directorFilms = getDirectorFilms(titleCrew, nconst);
    ...
    auto notAdultMovies = getNotAdultMovies(titleBasics, directorFilms);
    ...
    // не делаем проверку на empty, так как гарантированно вернутся
    // хотя бы оригинальные названия фильмов
    russianLocalNames = getRussianMoviesNames(titleAkas, notAdultMovies);
    titleAkas.close();
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        std::cerr <<
            e.what() << " Restart program with right input"
        << std::endl;
        return (-1);
    }
